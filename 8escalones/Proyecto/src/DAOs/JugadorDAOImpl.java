...
public class JugadorDAOImpl {
    ...
    public void actualizarPuntaje(Jugador jugador) {
        String query = "UPDATE jugador SET puntaje = ? WHERE id_jugador = ?";
        try (PreparedStatement statement = BaseDeDatos.prepareStatement(query)) {
            statement.setInt(1, jugador.getPuntaje());
...
/*     public void actualizarPuntaje(Jugador jugador) {
    // Suponiendo que usas JDBC
    String sql = "UPDATE jugadores SET puntaje = ? WHERE id_jugador = ?";
   ...
   /*  public ArrayList<Jugador> obtenerJugadoresConMenorPuntaje() {
        ...
        String query = """
            SELECT * FROM jugador 
            WHERE puntaje = (
                SELECT MIN(puntaje) FROM jugador
            )
        """;
