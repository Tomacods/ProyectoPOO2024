...
public class ControladorGameplay {
    ...
    private void traerTematica() { //arreglar para q setee la tematica correcta

        String tematica = Tematica.obtenerTematica(1).getNombre();
        vista.getjTextFieldTurnoActual().setText(tematica);
    }
    private void traerJugador() throws SQLException {

        String jugador = Jugador.obtenerJugador(1).getNombre();
        vista.getjLabelJugador().setText(jugador);
    }
...
private Jugador preguntarJugador(Jugador jugador, MultipleChoicePregunta pregunta) throws SQLException {
    System.out.println("Respuesta seleccionada por el jugador: " + rtaSelec); // Depuración
   ...
   /*  private void realizarPreguntas(int idEscalon) throws SQLException {
        ...
        ArrayList<Jugador>jugadores = Jugador.obtenerJugadores();
        for (Jugador jugador : jugadores) {
            String pregunta = pregMult.get(new Random().nextInt(pregMult.size())).getEnunciado();
           // MultipleChoicePregunta pregunta = pregMult.get(new Random().nextInt(pregMult.size()));
            //String preguntaStr = MultipleChoicePregunta.obtenerPreguntaMC(2).getEnunciado();
            int id_pregunta = MultipleChoicePregunta.obtenerPreguntaMC(2).getIdPregunta();
            vista.getjTextFieldPregunta().setText(pregunta);
            traerRespuestas(id_pregunta);
    ...
    //private Jugador preguntarJugador(Jugador jugador, MultipleChoicePregunta pregunta){
